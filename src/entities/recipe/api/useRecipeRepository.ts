...
export function useRecipeRepository(homeId: string | null) {
  ...
  /**
   * ⚡ Performance Optimization: Memoized callbacks to prevent cascading re-renders.
   * By stabilizing these function references, we ensure that:
   * 1. RecipeContext value remains stable, preventing app-wide re-renders.
   * 2. useInfiniteScroll doesn't recreate its IntersectionObserver unnecessarily.
   */
